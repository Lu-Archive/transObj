const _math = require('./utils/_math')
...
class TransObj {
    ...
    recursionSearchArray(obj, key, path) {
        for (let item in obj) {
            if (obj[item].constructor.name == 'Function' && obj[item].toString().indexOf('Array') > -1 && item === key) {
                path === undefined ? path = [item] : path.push(item)
                return {
                    state: 1,
            ...
            } else if (obj[item].constructor.name == 'Array') {
                path === undefined ? path = [item] : path.push(item)
    ...
    recursionObjFromArray(obj, obj1, oo, i) {
        for (let item in oo) {
            if (oo[item].constructor.name == 'String') {
                ...
                if (data.length > i) {
                    if (res.key === '') {
                        oo = this.setPathValue(oo, [item], data[i])
                    } else {
                        oo = this.setPathValue(oo, [item], this.getSourceValue(data[i], res.key))
                    }
                } else {
                    // 超过源数组长度设置为null
                    oo = this.setPathValue(oo, [item], null)
                }
...
module.exports = (obj, obj1, obj2) => {
    return new TransObj(obj, obj1, obj2)
}